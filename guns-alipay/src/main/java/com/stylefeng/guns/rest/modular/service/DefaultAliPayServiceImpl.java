...
/**
 * @Author: huhan
 * @Date 2020/8/22 8:05 上午
 * @Description
 * @Verion 1.0
 */
@Component
@Service(interfaceClass = AliPayServiceAPI.class,mock = "com.stylefeng.guns.api.alipay.api.AliPayServiceMock",filter = {"tracing"})
@Slf4j
public class DefaultAliPayServiceImpl implements AliPayServiceAPI {
    ...
    public String trade_precreate(String orderId) {
        ...
        String body = "购买电影票共花费" + orderInfo.getOrderPrice();

        // 商户操作员编号，添加此参数可以为商户操作员做销售统计
        String operatorId = "huhan";

        // (必填) 商户门店编号，通过门店号和商家后台可以配置精准到门店的折扣信息，详询支付宝技术支持
        String storeId = "huhan";
...
//        extendParams.setSysServiceProviderId("2088100200300400500");
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                filePath = String.format("/Users/huhan/MyDocuments/qrcode/qr-%s.png",
                        response.getOutTradeNo());
                String fileName = String.format("qr-%s.png", response.getOutTradeNo());
                log.info("filePath:" + filePath);
                //获取二维码，并上传到 ftp 服务器
                File qrCodeImge = ZxingUtils.getQRCodeImge(response.getQrCode(), 256, filePath);
                if (!ftpUtil.uploadFile(fileName, qrCodeImge)) {
                    filePath = "";
                    log.error("二维码上传失败");
                }
                break;
